...
function formatRunnLink(resource: RunnTriggerResource, record: { id?: string; personId?: string }): string {
		...
		const checkRunnForUpdates = async () => {
			...
			if (events.includes(EVENTS.CREATED) || events.includes(EVENTS.UPDATED)) {
				...
				if (resources.includes(RESOURCES.PEOPLE)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.PEOPLE, (p) => runnApi.people.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.PROJECTS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.PROJECTS, (p) => runnApi.projects.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.CLIENTS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.CLIENTS, (p) => runnApi.clients.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.ASSIGNMENTS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.ASSIGNMENTS, (p) => runnApi.assignments.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.ACTUALS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.ACTUALS, (p) => runnApi.actuals.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.CONTRACTS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.CONTRACTS, (p) => runnApi.contracts.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.USERS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.USERS, (p) => runnApi.users.fetchAll(p)));
				}
				if (resources.includes(RESOURCES.TEAMS)) {
					resultItemsMatched.push(...await checkForUpdates(RESOURCES.TEAMS, (p) => runnApi.teams.fetchAll(p)));
				}
			}
